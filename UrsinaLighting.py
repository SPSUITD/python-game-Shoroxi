...
if __name__ == "__main__":
    ...
    # objects
    # bar = Entity(model='Comb_Bar.obj', scale=(0.01,0.01,0.01), texture='white_cube', mode='triangle', ambientStrength=0.6)
    ground = LitObject(model = "plane", scale = 100, texture = texture, specularMap = specTexture, normalMap = normTexture, ambientStrength = 0.5, collider="box")
    fow = LitObject(model = "plane", position = (0, 20, 1), scale = 100, texture = texture, specularMap = specTexture, normalMap = normTexture, ambientStrength = 0.5, collider="box")
    cube = LitObject(model = "cube", position = (0, 0.5, 1), texture = "white_cube", specularMap = None, normalMap = None, ambientStrength = 0.5)

    # lights
    sun = LitDirectionalLight(direction = Vec3(0.5, -0.6, 1), intensity=0, color=color.gray)
    ...
    camera.fov = 100
    spotLight = LitSpotLight(direction=camera.ui)
    ...
    def update():
        ...
        spotLight.setDirection(camera.position)
    app.run()
